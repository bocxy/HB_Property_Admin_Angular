...
export class ViewSchemesComponent implements OnInit {
  ...
  schemesTableColumns: string[] = ['n_ID', 'v_DIVISION', 'v_SCHEME_CODE', 'v_SCHEME_NAME', 'n_TOTAL_DEVELOPED_UNITS', 'ACTION', 'WEBSITE_DATA','MASTER_DATA'];
  id: any;
    ...
    this.httpService.getAllSchemesData(1).subscribe(
      (response) => {
        ...
        if (response && Array.isArray(response.data)) {
          ...
          this.allSchemesDataSource.data = response.data.map((scheme: any, index: number) => ({
            ...scheme,
